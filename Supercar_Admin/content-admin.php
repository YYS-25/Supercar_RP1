...
<head>
  ...
  <script>
    function populateTextModal(id, section, content, image, lang, page) {
      ...
      document.getElementById('text-image').value = clientSource + image;
      document.getElementById('text-preview-image').src = clientSource + image;
    }
  </script>
</head>
<body>
<div class="container mt-4">
    <div class="card text-center" style="margin-top: 20px; padding: 24px 0px; background: linear-gradient(135deg, #fff07fff, #f97373ff); border-radius: 15px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3);">
      <h1 class="main-title">Gestion du Contenu</h1>
      <p class="text-muted" style="color: #333;">Modifier le contenu des pages du site web en bilingue.</p>
    </div>
              ...
              <?php $result = ${$page . '_' . $lang}; while ($row = $result->fetch_assoc()): ?>
                <tr>
                  ...
                  <td>    <?php if (!empty($row['image'])): ?>
                            <img src="<?= 'https://site-supercar.alwaysdata.net/' .  htmlspecialchars($row['image']) ?>" alt="" style="height:40px; width:60px;">
                          <?php endif; ?>
                  </td>
                  <td>
